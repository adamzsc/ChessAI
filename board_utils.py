...
class CNN_Regressor(nn.Module):
    ...
    def forward(self, x, extra_features):
        ...
        Args:
            x (Tensor): The input tensor for the CNN (the board).
            extra_features (Tensor): The input tensor for the extra features (e.g., material).
        """
        x = self.pool1(self.relu1(self.conv1(x)))
        x = self.pool2(self.relu2(self.conv2(x)))
        x = self.flatten(x)
        
        # Concatenate the flattened CNN output with the extra features
...
def get_control_channel(board,turn):
    ...
    return np.array(control_channel)